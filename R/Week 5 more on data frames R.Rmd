...
gap_data_wide <- gap_data_long %>%
  pivot_wider(
    id_cols = c(country, continent, year),
    names_from = metric,
    values_from = value
  )
...
We could also make the data even wider by passing multiple columns to the `values_from` parameter. Notice that when doing so, the column names update accordingly 
